...
from cv2 import imwrite,imdecode, IMREAD_UNCHANGED,imread
...
@torch.no_grad()
def infer_and_save_depth(input_file, instance, output_file, model_wrapper, image_shape, half, save):
    """
    ...
    # Load image
    imageLoad = load_image(input_file)
    height, width = imageLoad.size
    width = int(width/64)
    width = width * 64
    height = int(height/64)
    height = height*64
    # Resize and to tensor
    image = resize_image(imageLoad, (width,height))
    ...
    if save == 'npz' or save == 'png':
        ...
        instance.infer_time = end_time-start_time
        instance.save()
        ...
        bucket = client.get_bucket('robok-d695e.appspot.com')

        image = cv2.imread(output_file)

        with NamedTemporaryFile() as temp:
            temp.name = output_file
            iName = temp.name
            print(iName)
            cv2.imwrite(iName,image)
            print(os.path.exists(str(iName)))
            blob = bucket.blob("media/"+output_file)
            blob.upload_from_filename(iName)
            instance.result = "media/"+output_file
...
def main(checkpoint,image_shape, half, save, image,instance):

    # parser = argparse.ArgumentParser(description='PackNet-SfM inference of depth maps from images')
    # parser.add_argument('--halfimread', action="store_true", help='Use half precision (fp16)')
    # halfArg = parser.parse_args()
    # args.half = halfArg.half
    # Initialize horovod
    hvd_init()
    ...
    for fn in files[rank()::world_size()]:
        output = str(instance[count-1].id)+"_"+str(count)+"_output.jpg"
        infer_and_save_depth(
            fn, instance[count-1] , output, model_wrapper, image_shape, half, save)
        count = count+1
